...
if [ "${vh_enable}" = "1" ] ; then
	...
	auth=$(grep -E "^clientAuthorization=" /jffs/softcenter/etc/vhusbd/vhusbd.ini)
	auth_script="/tmp/vhusbd_auth.sh"
	if [ -n "$vh_password" ] ; then
		logg "增加密码验证"
    		if [ -z "$auth" ] ; then
        		echo 'clientAuthorization=/tmp/vhusbd_auth.sh "$VENDOR_ID$" "$PRODUCT_ID$" "$CLIENT_ID$" "$CLIENT_IP$" "$PRODUCT_SERIAL$" "$PASSWORD$" "$DEVPATH$" "$NICKNAME$" "$NUM_BINDINGS$"' >> /jffs/softcenter/etc/vhusbd/vhusbd.ini
    		fi
